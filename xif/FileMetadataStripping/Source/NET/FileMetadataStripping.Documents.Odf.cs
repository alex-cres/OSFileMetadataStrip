...
namespace OutSystems.NssFileMetadataStripping {
	...
	public partial class CssFileMetadataStripping
	{
		...
		private static RCFileMetadataResultRecord StripFlatOdfMetadata(byte[] rawFile)
		{
			try
			{
				XDocument xdoc;
				...
				using (var output = new MemoryStream())
				{
					...
					r.ssSTFileMetadataResult.ssExtractedMetadata = json;
					r.ssSTFileMetadataResult.ssRemovedEntryCount = n;
					r.ssSTFileMetadataResult.ssIsPassthrough     = false;
					return r;
				}
			}
		...
		/// <summary>
		/// Returns true when the byte buffer looks like an XML document — after an
		/// optional UTF-8 BOM and leading whitespace, the first non-whitespace byte is
		/// '&lt;'. Used as a cheap prefilter for the Flat ODF and WordML byte-scanners.
		/// </summary>
