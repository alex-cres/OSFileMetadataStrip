...
namespace OutSystems.NssFileMetadataStripping {
	...
	public partial class CssFileMetadataStripping
	{
		...
		// boundary.
		private static readonly JsonSerializerOptions _gdiJsonOpts = new JsonSerializerOptions
		{
		...
		/// </summary>
		private static RCFileMetadataResultRecord StripImageMetadataWithGdi(byte[] rawFile)
		{
			...
			if (!IsGdiSupportedFormat(detected))
			{
				...
				er.ssSTFileMetadataResult.ssExtractedMetadata = errNode.ToJsonString(_gdiJsonOpts);
			...
			try
			{
				...
				using (var img = Image.FromStream(input, useEmbeddedColorManagement: false, validateImageData: false))
				{
					...
					var propertyIds = img.PropertyIdList != null ? img.PropertyIdList.ToArray() : Array.Empty<int>();
					...
					int frameCount;
					if (IsMultiFrameSaveable(img, out frameDim, out frameCount) && frameCount > 1
					...
					r.ssSTFileMetadataResult.ssExtractedMetadata = count > 0 ? extracted.ToJsonString(_gdiJsonOpts) : "[]";
			...
			catch (Exception ex)
			{
				...
				var er = new RCFileMetadataResultRecord(null);
				er.ssSTFileMetadataResult.ssCleanFile         = rawFile;
				er.ssSTFileMetadataResult.ssExtractedMetadata = note.ToJsonString(_gdiJsonOpts);
				er.ssSTFileMetadataResult.ssRemovedEntryCount = 0;
				er.ssSTFileMetadataResult.ssIsPassthrough     = false;
				return er;
			}
		...
		{
			return format == "Jpeg" || format == "Png" || format == "Gif" || format == "Bmp" || format == "Tiff";
		}
		...
		private static byte[] SaveMultiFrameStripped(Image img, FrameDimension dim, int frameCount, string detected)
		{
			...
			using (var outStream = new MemoryStream())
			{
				...
				for (int i = 1; i < frameCount; i++)
				{
					...
					var frameIds = img.PropertyIdList != null ? img.PropertyIdList.ToArray() : Array.Empty<int>();
		...
		private static readonly Dictionary<int, string> _gdiPropertyNames = new Dictionary<int, string>
		{
			// TIFF / EXIF core
		...
		{
			string name;
			return _gdiPropertyNames.TryGetValue(id, out name)
				? name
