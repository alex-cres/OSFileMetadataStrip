...
namespace OutSystems.NssFileMetadataStripping {
	...
	public partial class CssFileMetadataStripping
	{
		...
		private static RCFileMetadataResultRecord StripEpubMetadata(byte[] rawFile)
		{
			try
			{
		...
		private static RCFileMetadataResultRecord StripOraMetadata(byte[] rawFile)
		{
			try
			{
				var zipMs = new MemoryStream();
		...
		private static RCFileMetadataResultRecord StripCfbfMetadata(byte[] rawFile)
		{
			try
			{
				var ms = new MemoryStream();
				...
				r.ssSTFileMetadataResult.ssExtractedMetadata = count > 0 ? extractedRoot.ToJsonString() : "[]";
				r.ssSTFileMetadataResult.ssRemovedEntryCount = count;
				r.ssSTFileMetadataResult.ssIsPassthrough     = false;
				return r;
			}
		...
		private static RCFileMetadataResultRecord StripRtfMetadata(byte[] rawFile)
		{
			try
			{
