...
public partial class CssFileMetadataStripping
{
    ...
    /// </summary>
    private static RecFileMetadataResult StripImageMetadataWithFallback(byte[] rawFile)
    {
        if (rawFile is null || rawFile.Length == 0)
            return Passthrough(rawFile ?? System.Array.Empty<byte>());
    ...
    internal static string? DetectImageFormatByMagicBytes(byte[] b)
    {
        ...
        // security-signal downgrade (the file DOES carry EXIF/GPS and did NOT get
        // stripped). See regression detector v1 report for the full trace.
    ...
    private static RecFileMetadataResult StripImageMetadataWithMagick(byte[] rawFile)
    {
