...
internal static partial class TestHelpers
{
    ...
    /// <summary>
    /// Creates a PNG file requested in a specific subformat — PNG8 (8-bit indexed),
    /// PNG24 (24-bit RGB), PNG32 (32-bit RGBA), PNG48 (48-bit RGB), PNG64 (64-bit RGBA),
    /// or PNG00 (inherit subformat from source). On the Q8 build of Magick.NET the
    /// PNG48 and PNG64 requests are effectively downsampled to 8-bit-per-channel
    /// output, but the encoder still writes a valid PNG signature.
    /// </summary>
    ...
    /// <summary>
    /// Creates a DIB (Windows Device Independent Bitmap) — the same pixel layout as
    /// BMP but WITHOUT the 14-byte <c>BITMAPFILEHEADER</c> (no "BM" prefix). The
    /// output starts with a <c>BITMAPINFOHEADER</c> whose first four bytes encode
    /// the header size (typically 40 → <c>0x28 0x00 0x00 0x00</c>). Because DIB has
    /// no unique start-of-file magic, our <c>DetectCategory()</c> relies on
    /// <see cref="MagickImageInfo"/> to identify it.
    /// </summary>
    ...
    /// <summary>
    /// Creates a JPEG-2000 code-stream file (J2C / J2K / JPT variants). Code streams
    /// start with the <c>SOC</c> marker <c>0xFF 0x4F</c> followed by the <c>SIZ</c>
    /// marker <c>0xFF 0x51</c>, distinguishing them from the JP2 file-format wrapper.
    /// </summary>
    ...
    /// <summary>
    /// Creates a PSB file (Adobe Large Document Format). PSB shares the 8BPS Photoshop
    /// header with PSD but uses version 2 instead of version 1 — the first six bytes
    /// are <c>0x38 0x42 0x50 0x53 0x00 0x02</c>.
    /// </summary>
    ...
    /// <summary>
    /// Creates a DCX file (ZSoft multi-page Paintbrush). DCX starts with the 4-byte
    /// magic <c>0xB1 0x68 0xDE 0x3A</c> (little-endian 32-bit signature
    /// <c>0x3ADE68B1</c>) and wraps one or more PCX images.
    /// </summary>
    ...
    /// <summary>
    /// Creates a file written with the MSVG (Magick internal SVG renderer) format.
    /// The output is an SVG document — the same XML text the SVG renderer would emit —
    /// so our <c>IsSvgFile</c> detector catches it and routes it through the dedicated
    /// SVG strip path rather than through Magick.NET.
    /// </summary>
    ...
    /// <summary>Creates an animated PNG (APNG) using MagickImageCollection.
    /// Without the 'apng:' read specifier, MagickImageCollection may read only the first frame.</summary>
    ...
    internal static byte[] CreateApng(int frameCount = 2)
    {
        ...
        // IHDR: 10×10, 8-bit RGB (colour type 2)
        ...
        for (int f = 0; f < frameCount; f++)
        {
            ...
            // fcTL: seq(4) w(4) h(4) x_off(4) y_off(4) delay_num(2) delay_den(2) dispose(1) blend(1)
            var fctl = new byte[26];
            ApngWrite32(fctl, 0, seq++);
            ApngWrite32(fctl, 4, (uint)W);
            ApngWrite32(fctl, 8, (uint)H);
            fctl[16] = 0; fctl[17] = 1;  // delay_num = 1
            fctl[18] = 0; fctl[19] = 10; // delay_den = 10 → 100 ms per frame
            ApngWriteChunk(ms, new byte[] { 0x66, 0x63, 0x54, 0x4C }, fctl);

            // Raw pixels: 1 filter byte (0 = None) + R G B per pixel, per row
            ...
            if (f == 0)
            {
                // First frame: standard IDAT
                ApngWriteChunk(ms, new byte[] { 0x49, 0x44, 0x41, 0x54 }, compressed);
            }
            else
            {
                // Subsequent frames: fdAT (seq number prefix + compressed data)
    ...
    {
        // zlib header + DEFLATE stored (uncompressed) blocks + Adler-32 checksum.
        // DEFLATE stored mode is valid PNG compression and requires no compression library.
        using var ms = new MemoryStream();
        ms.WriteByte(0x78); // CMF: deflate, 32 KB window
        ms.WriteByte(0x01); // FLG: check (0x7801 % 31 == 0) ✓
        int remaining = data.Length, offset = 0;
        do
        {
            int blockLen = Math.Min(remaining, 65535);
            bool isFinal = blockLen == remaining;
            ms.WriteByte(isFinal ? (byte)0x01 : (byte)0x00); // BFINAL | BTYPE=00 (stored)
