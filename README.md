...
| Flat ODF (single-file XML) | FODT, FODS, FODP | Same `dc:*` and `meta:*` elements as the ZIP-based ODF path — detected by the `<office:document>` root in the OASIS office namespace and processed through the shared `ExtractAndClearOdfMetadata` helper |
| Word 2003 XML (WordProcessingML) | XML (Word 2003) | Every child of `<o:DocumentProperties>` (Author, LastAuthor, Company, Manager, Title, Subject, Keywords, Description, Category, Template, HyperlinkBase, Application, AppVersion, TotalTime, LastPrinted, Created, LastSaved, revision counters) and every child of `<o:CustomDocumentProperties>`. When `StripBodyAuthors = True`: also blanks `w:author` and `aml:author` attributes on tracked-change and comment elements. |
...
| ODF (ODT, ODS, ODP, OTT, OTS, OTP, ODG, OTG, ODC, ODF, ODB, ODI) | System.IO.Compression (BCL) | dc:creator, dc:title, dc:description, dc:subject, meta:initial-creator, meta:generator, meta:editing-cycles, meta:editing-duration, and all meta:user-defined properties in meta.xml. Templates, drawings, charts, formulas, databases, and images all match via the `application/vnd.oasis.opendocument.*` mimetype prefix and reuse the same strip path. |
| Flat ODF (FODT, FODS, FODP) | System.Xml.Linq (BCL) | Detected via the `<office:document>` root in the OASIS office namespace. Parsed with `XDocument.Load` and processed by the shared `ExtractAndClearOdfMetadata` helper — same strip surface as the ZIP-based ODF path. |
| Word 2003 XML (WordProcessingML .xml) | System.Xml.Linq (BCL) | Detected via the `<w:wordDocument>` root in the `http://schemas.microsoft.com/office/word/2003/wordml` namespace. `<o:DocumentProperties>` and `<o:CustomDocumentProperties>` children removed; `w:author` / `aml:author` attributes blanked when `StripBodyAuthors = True`. |
