...
public class ImageTests
{
    ...
    [Fact]
    public void StripFileMetadata_WithAnimatedGif_CommentIsStrippedFromAllFrames()
    {
        var input = TestHelpers.CreateAnimatedGifWithComment("injection payload");

        var result = _sut.StripFileMetadata(input, false);

        using var images = new MagickImageCollection(result.CleanFile);
        foreach (var frame in images)
            Assert.True(string.IsNullOrEmpty(frame.Comment));
    }

    [Fact]
    public void StripFileMetadata_WithAnimatedGif_CommentIsCapturedInExtractedMetadata()
    {
        var input = TestHelpers.CreateAnimatedGifWithComment("injection payload");
        ...
        Assert.Contains("comment", result.ExtractedMetadata);
        Assert.Contains("injection payload", result.ExtractedMetadata);
    }
